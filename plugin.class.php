...
class VC_Image_Banner_Free
{
	
	function __construct()
	{
		add_action('vc_before_init', array($this, 'wdo_option_settings'));
		add_shortcode( 'image-banner-vc-free', array( $this, 'render_image_hover_shortcode' ) );
	...
	function wdo_option_settings(){
		...
		$ihe_main_var = array(
			...
			"base" => "image-banner-vc-free",
			"category" => __('by aloisio'),
			"description" => __('Insert Images banner'),
	...
	function render_image_hover_shortcode($attrs, $content = null){ 
		...
		if(!empty($ihe_image)){
		    $imgList = explode(',',$ihe_image);
            $image_url = [];
		    foreach ($imgList as $img){
		        array_push($image_url,wp_get_attachment_url($img));
            }
        }

        if(!empty($link_anterior)){
            $anterior_link_url =  vc_build_link( $link_anterior );
        }

        if(!empty($link_proximo)){
            $proximo_link_url =  vc_build_link( $link_proximo );
        }
	...
	function showVcVersionNotice() {
        $plugin_name = 'Image Banner Visual Composer Extension';
        echo '
        <div class="updated">
